...
import { ObservableObject, getObservable } from './ObservableObject.mjs'
...
/* 	
	...
	@param auth - async function(socket, id) throws Error if auth fails
*/
const connectionListener = (server, auth) => {
	const io = new Server(server)

	io.on('connection', (socket) => {

		const authHandler = async (id, sourceId, ack) => {

			let data
			try {
				data = await auth(socket, id)  // call auth, returns initial data onject
			}
			catch(err) {
				return ack({ status: 'error', message: err.message }) // auth failed. tell client.
			}

			// validation ok, find or build observable data
			let observed = getObservable(id)
			if(!observed) { 
				observed = new ObservableServer(id, data, { authoritative: true })
			}

			// hook authenticated socket up to observable
...
export {
	ObservableServer, connectionListener
}
