...
class TextField extends SargassoComponent {
	constructor (element, options) {
		...
		this.renderAttributes = ['type', 'label', 'value', 'helper-text', 'prefix-text', 'suffix-text', 'icon-leading-icon', 'icon-trailing-icon', 'rows', 'cols', 'minlength', 'maxlength', 'disabled', 'required', 'max', 'min', 'step', 'autocomplete', 'list', 'maxlength', 'minlength', 'multiple', 'pattern', 'readonly', 'current-password', 'new-password', 'inputmode', 'autocorrect']
	...
	start () {
		...
		this.on('change keyup', 'textarea', (e) => {
			this._hostElement.setAttribute('value',e.target.value)
			if(e.type === 'change') {
				this._hostElement.dispatchEvent(new Event(e.type,{composed:true}))
			}
				...
				markup = (args) => html`
					<label class="${inputClasses}">
						...
						<input
							...
							inputmode=${ifDefined(args.attributes.inputmode)}
							autocorrect=${ifDefined(args.attributes.autocorrect)}
							...
							?current-password=${args.attributes['current-password']}
							?new-password=${args.attributes['new-password']}
						>
						${suffix(args)}
						${iconTrailing(args)}
						<span class="mdc-line-ripple"></span>
