var SargassoModule = (function (exports) {
	...
	**/
	...
	class ObservableObjectWatcher extends ObserverSubscriptionManager {
		...
		unRegister (id) {
			if (!this.registeredObservableObjects[id]) {
				throw (new Error('ObservableObject unRegister ' + id + ' does not exist'))
			}
		...
		notify (id, property, source) {
			...
			observers.forEach((observer) => {
				if (observer.observableChange) {
					observer.observableChange.apply(observer, [id, property, source]);
				}
			});
		}
	}
	...
	**/
	...
	class Sargasso {
		...
		*************************************************/
		...
		observableObjectStart (id, data, property) {
			theObservableObjectWatcher.subscribe(this, id, data);
			this.observables[id] = theObservableObjectWatcher.getObservableObject(id);
		}

		observableObjectStop (id) {
